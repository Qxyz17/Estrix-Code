...
async function initProject(skipPrompt = false, windowContext = null) {
  ...
  if (!result || result.length === 0) {
    console.log('[Estrix Code] 用户取消了目录选择');
    return { success: false, message: '用户取消了目录选择' };
  }

  const selectedDir = result[0];
  console.log('[Estrix Code] 用户选择目录:', selectedDir);
  const tStart = Date.now();
  const stepLog = (msg) => logWithFile(providerId, '[Estrix Code][耗时] ' + msg + ' +' + (Date.now() - tStart) + 'ms');
  ...
  if (sessionStore) {
    ...
    if (sessionStore.state.currentSessionId) {
      ...
      console.log(`[Estrix Code] 已保存会话 ${sessionStore.state.currentSessionId} -> ${selectedDir}`);
    } else {
      ...
      if (sessionId) {
        ...
        console.log(`[Estrix Code] 从URL提取会话ID并保存: ${sessionId} -> ${selectedDir}`);
      } else {
        // 无法获取会话ID，暂存项目目录，等待URL变化后绑定
        sessionStore.state.pendingProjectDir = selectedDir;
        console.log(`[Estrix Code] 暂存项目目录 ${selectedDir}，等待会话ID出现后绑定`);
      }
    }
  }
  ...
  if (provider && typeof provider.getPromptTemplate === 'function') {
    try {
      ...
      console.warn('[Estrix Code] 调用 provider.getPromptTemplate 失败:', err.message);
    }
  }
  ...
  if (!templateContent && templatePath) {
    try {
      ...
      console.error('[Estrix Code] 读取提示词模板失败:', err.message);
  ...
  if (!templateContent) {
    ...
    try {
      templateContent = fs.readFileSync(templatePath, 'utf-8');
      console.warn('[Estrix Code] 未找到平台模板，使用默认模板:', templatePath);
    } catch (err) {
      console.error('[Estrix Code] 读取默认模板失败:', err.message);
      return { success: false, message: '读取默认提示词模板失败: ' + err.message };
    }
  }

  console.log('[Estrix Code] 已读取提示词模板:', templatePath);

  stepLog('读取模板完成');
  // 读取工具 API 类型定义（从 d.ts 文件读取，避免与模板重复维护）
  let toolApiTypes = '';
  try {
    toolApiTypes = fs.readFileSync(path.join(__dirname, '..', '..', 'tools', 'estrix-tools.d.ts'), 'utf-8');
  } catch (err) {
    console.error('[Estrix Code] 读取 estrix-tools.d.ts 失败:', err.message);
  }
  ...
  // 读取项目介绍（ESTRIX.md）
  let projectIntro = '';
  const estrixMdPath = path.join(selectedDir, '.estrixCode', 'ESTRIX.md');
  if (fs.existsSync(estrixMdPath)) {
    try {
      projectIntro = fs.readFileSync(estrixMdPath, 'utf-8');
      console.log('[Estrix Code] 已读取 ESTRIX.md 内容');
    } catch (err) {
      console.error('[Estrix Code] 读取 ESTRIX.md 失败:', err.message);
    }
  }
