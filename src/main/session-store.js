...
function createSessionStore(profileId, storeDir, windowState) {
  ...
  function readSessionStore() {
    try {
      ...
      console.error('[Estrix Code] 读取会话存储失败:', err.message);
    }
    return {};
  }
  ...
  function writeSessionStore(store) {
    try {
      ...
      console.log('[Estrix Code] 会话存储已保存:', STORE_FILE);
    } catch (err) {
      console.error('[Estrix Code] 写入会话存储失败:', err.message);
    }
  }
  ...
  function handleUrlChange(url, targetWindow) {
    ...
    if (sessionId) {
      ...
      console.log('[Estrix Code][' + profileId + '] 当前会话ID: ' + sessionId);
      ...
      if (state.pendingProjectDir) {
        ...
        console.log('[Estrix Code][' + profileId + '] 暂存目录已绑定');
        return;
      }
