...
module.exports = {
  ...
  async isResponseComplete() {
    ...
    if (stopBtnVisible) {
      ...
      if (generatingMs < MIN_GENERATING_MS) {
        ...
        console.log('[' + new Date().toISOString() + '] [Estrix Code] ChatGPT 停止按钮仅存在 ' + generatingMs + 'ms（<' + MIN_GENERATING_MS + 'ms），忽略本次完成信号');
        return false;
      }
      console.log('[' + new Date().toISOString() + '] [Estrix Code] ChatGPT 回复完成，等待 500ms 后解析');
      await new Promise(resolve => setTimeout(resolve, 500));
      console.log('[' + new Date().toISOString() + '] [Estrix Code] ChatGPT 500ms 等待结束');
      return true;
    }
  ...
  // 提取代码块的语言标记
  // ChatGPT 代码块的语言标签在 header 里（如 <svg/>estrix），不在 class 中。
