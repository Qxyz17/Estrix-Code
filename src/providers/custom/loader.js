...
if (isRenderer) {
  ...
  const arg = argv.find(a => a.startsWith('--estrix-user-data='));
  if (arg) rendererUserDataPath = arg.slice('--estrix-user-data='.length);
}
...
function getCacheHolder() {
  ...
  if (!process.__estrixCustomProvidersCache ||
      process.__estrixCustomProvidersCache.key !== key) {
    process.__estrixCustomProvidersCache = { key, providers: null };
  }
  return process.__estrixCustomProvidersCache;
}
