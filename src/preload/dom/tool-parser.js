...
function tryParseToolCall(content) {
  ...
  if (!parsed.toolName && !parsed.tool) {
    console.log('[Estrix Code] 缺少 toolName/tool 字段, 完整对象:', JSON.stringify(parsed));
    return null;
  }
  ...
  console.log('[Estrix Code] ✅ 解析成功, toolName=' + result.toolName + ', params=' + JSON.stringify(result.params));
  return result;
}
