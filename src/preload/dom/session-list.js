...
async function renderSessions() {
  const listContainer = document.getElementById('estrix-session-list');
  if (!listContainer) return;

  try {
    if (!window.electronAPI || !window.electronAPI.listSessions) {
      listContainer.innerHTML = '<div class="estrix-session-empty">API 不可用</div>';
      return;
    }

    const result = await window.electronAPI.listSessions();
    if (!result.success) {
      listContainer.innerHTML = '<div class="estrix-session-empty">加载失败</div>';
      return;
    }

    const sessions = result.sessions || [];
    if (sessions.length === 0) {
      listContainer.innerHTML = '<div class="estrix-session-empty">暂无会话</div>';
      return;
    }

    listContainer.innerHTML = sessions.map((sessionId) => `
      <div class="estrix-session-item" data-session-id="${escapeHtml(sessionId)}">
    ...
    // 绑定点击事件
    listContainer.querySelectorAll('.estrix-session-item').forEach((item) => {
    ...
    console.error('[Estrix Code] 渲染会话列表失败:', err);
    listContainer.innerHTML = '<div class="estrix-session-empty">加载出错</div>';
  }
}
...
async function handleNavigateSession(sessionId) {
  ...
  try {
    ...
    if (result.success) {
      console.log('[Estrix Code] 已导航到会话:', sessionId);
    ...
    console.error('[Estrix Code] 导航到会话失败:', err);
...
async function handleInitProject() {
  ...
  try {
    ...
    console.error('[Estrix Code] 初始化项目失败:', err);
