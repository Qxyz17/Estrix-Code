...
async function setInputContent(input, msg) {
  try {
    ...
    console.error('[Estrix Code] 设置输入框内容失败:', err.message);
    return false;
  }
}
/**
...
async function sendToChat(msg, tag, fixedDelay, afterSent) {
  ...
  if (!input) {
    console.log('[Estrix Code] 找不到输入框，无法发送消息');
    return false;
  }
  ...
  console.log('[Estrix Code] 消息已填入输入框，等待 ' + sendDelay + 'ms 后发送...');
  setTimeout(function() {
    console.log('[Estrix Code] 等待结束，开始触发发送');
    triggerSend(input);
    console.log('[Estrix Code] 已触发发送, ' + (tag || '') + ', 长度=' + msg.length);
...
function sendToolResultToChat(toolCall, result) {
  ...
  console.log('[Estrix Code] 回传工具结果, 消息长度=' + msg.length);
...
function sendCombinedJsResultsToChat(results) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (item && item.result && item.result.success) {
      ...
      msg += '请修正 JavaScript 代码后重新输出完整的 ' + BT + BT + BT + 'estrix 代码块。';
    }
    msg += sep + sep;
  }

  console.log('[Estrix Code] 回传 JS 汇总执行结果, 消息长度=' + msg.length);
...
async function sendInitialPromptToInput() {
  ...
  console.log('[Estrix Code] 初始提示已填入，随机等待 ' + sendDelay + 'ms 后发送...');
  setTimeout(function() {
    console.log('[Estrix Code] 等待结束，开始发送初始提示');
    triggerSend(input);
    state.pendingInitialPrompt = false;
  }, sendDelay);
...
function waitForInitialPromptAndSend() {
  ...
  console.log('[' + new Date().toISOString() + '] [Estrix Code] 开始等待输入框出现（最多 ' + maxAttempts + ' 次，每次 500ms）');
  ...
  const checkInterval = setInterval(() => {
    ...
    if (attempts > maxAttempts) {
      ...
      state.pendingInitialPrompt = false;
      console.log('[' + new Date().toISOString() + '] [Estrix Code] 等待输入框超时，放弃发送初始提示');
      return;
    }

    const found = !!findInputArea();
    if (attempts === 1 || attempts % 5 === 0 || found) {
      console.log('[' + new Date().toISOString() + '] [Estrix Code] 等待输入框第 ' + attempts + ' 次检查, 输入框=' + (found ? '找到' : '未找到'));
    }
    if (found) {
      clearInterval(checkInterval);
  ...
  if (provider && typeof provider.triggerSend === 'function') {
    ...
    if (result && typeof result.then === 'function') {
      result.then(function (ok) {
        if (ok) {
          console.log('[Estrix Code] 已通过站点原生发送触发');
        } else {
...
function fallbackSend(provider, input) {
  ...
  if (provider && typeof provider.findSendButton === 'function') {
    ...
    if (btn) {
      btn.click();
      console.log('[Estrix Code] 已点击发送按钮');
      return;
    }
  }
  ...
  if (input) {
    ...
    console.log('[Estrix Code] 已通过 Enter 键触发发送 (未找到发送按钮)');
  }
}
/**
...
ipcRenderer.on('initial-prompt', (_event, content) => {
  console.log('[' + new Date().toISOString() + '] [Estrix Code] 收到 initial-prompt 事件, content长度=' + (content || '').length);
  ...
  if (state.pendingInitialPrompt && state.initialPromptContent) {
    try {
      const input = findInputArea();
      console.log('[' + new Date().toISOString() + '] [Estrix Code] 首次查找输入框结果=' + (input ? '找到' : '未找到'));
      if (input) {
        sendInitialPromptToInput();
      } else {
        // 等待输入框出现
      ...
      console.error('[' + new Date().toISOString() + '] [Estrix Code] initial-prompt 处理异常:', e.message);
    }
  }
});
}
