...
async function triggerManualParseAttention() {
  const btn = document.getElementById('estrix-btn-manual-parse');
  if (btn) {
    btn.classList.remove('estrix-btn-attention');
    // 强制回流以重新触发动画
    void btn.offsetWidth;
    btn.classList.add('estrix-btn-attention');
    // 动画结束后移除类，避免状态残留
    setTimeout(() => {
      btn.classList.remove('estrix-btn-attention');
    }, 3500);
...
async function handleManualParse() {
  ...
  try {
    ...
    console.error('[Estrix Code] 手动解析出错:', err);
...
async function executeJsBlocksWithRetry(initialBlocks, markdown, force) {
  ...
  for (let attempt = 0; attempt <= MAX_JS_RETRY; attempt++) {
    ...
    if (attempt < MAX_JS_RETRY) {
      console.log('[' + new Date().toISOString() + '] [Estrix Code] ⏳ 代码不完整，等待 1 秒后重新获取并重试（' + (attempt + 1) + '/' + MAX_JS_RETRY + '）...');
      await sleep(1000);
      console.log('[' + new Date().toISOString() + '] [Estrix Code] ⏳ 等待结束，开始第 ' + (attempt + 1) + ' 次重试');
  ...
  if (stillIncomplete) {
    console.log('[' + new Date().toISOString() + '] [Estrix Code] ⚠️ 代码不完整，已重试 ' + MAX_JS_RETRY + ' 次仍失败，将报错回传 AI');
  }
...
function processLatestAIResponse(retryCount = 0, force = false) {
  ...
  if (messages.length === 0) {
    console.log('[Estrix Code] 未找到 AI 消息节点');
    return;
  }
  ...
  if (!lastMessage || !markdown) {
    console.log('[Estrix Code] 未找到有内容的 AI 回复');
    return;
  }
  ...
  if (providerForUser && typeof providerForUser.isUserMessage === 'function' && providerForUser.isUserMessage(lastMessage)) {
    ...
    console.log('[Estrix Code] ⏭ 跳过用户消息（包含系统提示词示例）');
    return;
  }

  // 优先检测 JS 工具代码块（estrix 代码块 / 调用工具函数的 js 代码块）
  ...
  if (jsBlocks.length > 0) {
    ...
    if (force) {
      ...
      processedMessages.add(lastMessage);
      console.log('[Estrix Code] 手动解析模式，跳过稳定性校验');
    ...
    if (!rec || rec.snapshot !== snapshot || rec.blocksSig !== blocksSig) {
      ...
      console.log('[Estrix Code] ⏳ 检测到 JS 工具代码块，流式渲染中，等待稳定...');
      return; // 不标记 processed，稳定后执行
    }

    // 内容一致：需稳定满窗口确认
    if (now - rec.lastChange < JS_STABILITY_WINDOW) {
      console.log('[Estrix Code] ⏳ JS 代码块稳定中（等待 ' + JS_STABILITY_WINDOW + 'ms 确认）...');
      return;
    }
    ...
    processedMessages.add(lastMessage);
    console.log('[Estrix Code] ✅ 代码块稳定，检测到 JS 工具代码块（' + jsBlocks.length + ' 个），开始执行');
    // 正确使用 estrix 代码块，重置 XML 提示计数
    xmlHintCount = 0;
  ...
  if (!force) {
    ...
    if (!rec || rec.snapshot !== snapshot) {
      ...
      ensureStabilityTimer();
      console.log('[Estrix Code] ⏳ 文本内容渲染中，等待稳定（防流式中途漏检）...');
      return;
    }
    if (now - rec.lastChange < JS_STABILITY_WINDOW) {
      console.log('[Estrix Code] ⏳ 文本内容稳定中（等待 ' + JS_STABILITY_WINDOW + 'ms 确认）...');
      return;
    }
    jsStability.delete(lastMessage);
  }
  ...
  if (codeEl && hasOnlyCodeContent(markdown)) {
    ...
    console.log('[Estrix Code] 提取方式: pre code 元素（整条回复仅代码块）');
  } else {
    ...
    console.log('[Estrix Code] 提取方式: 完整文本(剔除工具栏)');
  }
  ...
  console.log('[Estrix Code] 回复文本长度: ' + text.length + (looksToolish ? '（疑似工具内容）' : '（普通文本）'));
  if (looksToolish) {
    console.log('[Estrix Code] 回复完整内容(原文):');
    console.log(text);
    console.log('[Estrix Code] 回复完整内容(转义显示):');
  ...
  if (!force && !isJsonBalanced(text)) {
    if (retryCount < MAX_RETRY_COUNT) {
      console.log('[Estrix Code] ⏳ JSON 不完整(疑似流式未结束)，' + (retryCount + 1) + '/' + MAX_RETRY_COUNT + ' 次延迟重试, 当前长度=' + text.length + '...');
      setTimeout(() => processLatestAIResponse(retryCount + 1), RETRY_INTERVAL);
      return; // 不标记 processed，允许重试
    }
    console.log('[Estrix Code] ⚠️ JSON 持续不完整（20次重试仍截断），放弃本次处理，当前长度=' + text.length);
    // 回传 AI，让它重新完整输出
  ...
  if (toolCall) {
    ...
    if (!available) {
      console.log('[Estrix Code] ⚠️ 工具不存在: ' + toolCall.toolName + ', 可用工具: ' + toolNamesList());
      // 回传 AI，告知工具不存在
      sendToolResultToChat(
        toolCall,
    ...
    console.log('[Estrix Code] ✅ 工具存在: ' + toolCall.toolName + ', 开始执行');
    notifyToolCallDetected(toolCall);
    handleToolCall(toolCall);
  } else {
    // JSON 工具调用未解析到，再检测 XML 格式的工具调用
    // 诊断：打印 XML 检测相关状态（text 和 innerHTML）
    console.log('[Estrix Code] [XML诊断] text长度=' + text.length + ', 开头100字符=' + JSON.stringify(text.slice(0, 100)));
    console.log('[Estrix Code] [XML诊断] markdown.innerHTML长度=' + (markdown.innerHTML || '').length + ', 开头200字符=' + JSON.stringify((markdown.innerHTML || '').slice(0, 200)));
    console.log('[Estrix Code] [XML诊断] 是否有 pre code 元素=' + !!markdown.querySelector('pre code'));
    // 精准判断：
    ...
    if (hasAntmlXml || (hasXmlInvoke && (hasXmlClose || hasXmlParam))) {
      ...
      if (xmlHintCount >= XML_HINT_MAX) {
        ...
        console.log('[Estrix Code] ⚠️ 已连续提示 ' + xmlHintCount + ' 次 XML 格式，停止发送提示语');
        return;
      }
      xmlHintCount++;
      console.log('[Estrix Code] ⚠️ 检测到 XML 格式工具调用（第 ' + xmlHintCount + ' 次提示），提示 AI 改用 estrix 代码块');
      const BT = String.fromCharCode(96);
      sendMessageToChat(
        '请使用' + BT + BT + BT + 'estrix' + BT + BT + BT + ' 代码块进行工具调用，不要使用 XML invoke 格式。',
        'XML工具调用提示'
      );
      return;
    }
    ...
    if (looksToolish) {
      ...
      console.log('[Estrix Code] ⚠️ 回复疑似工具调用但未被识别（JS 代码块未匹配 / JSON 解析失败）');
      ...
      if (pres.length > 0) {
        for (const p of pres) {
          ...
          const lang = (providerForLang && typeof providerForLang.getCodeBlockLanguage === 'function')
            ...
            : '';
          console.log('[Estrix Code] [诊断] 代码块 language=' + (lang || '(无)') + ', 内容前80字符=' + ((p.textContent || '').trim().slice(0, 80)));
        }
      } else {
        console.log('[Estrix Code] [诊断] 消息中没有任何 pre 代码块');
      }
    } else {
      console.log('[Estrix Code] ℹ️ 正常文本回复，未检测到工具调用（无需处理）');
...
function notifyToolCallDetected(toolCall) {
  ...
  // 更新预览区域显示检测到的工具调用
  const preview = document.getElementById('estrix-cmd-preview');
  if (preview) {
    preview.textContent = `[工具] ${toolCall.toolName}\n参数: ${JSON.stringify(toolCall.params, null, 2)}`;
  }
  // 闪烁状态徽章
  flashBadge('Estrix Code - 工具调用检测到');
}
/**
...
function notifyJsScriptDetected(code) {
  ...
  flashBadge('Estrix Code - JS 工具脚本检测到');
}
/**
...
async function handleJsToolScript(code) {
  ...
  console.log('[Estrix Code] [诊断] 即将执行的代码(JSON转义): ' + JSON.stringify(code));
  try {
    const result = await window.electronAPI.executeJs(code, callId);

    const resultSection = document.getElementById('estrix-result-section');
    const resultStatus = document.getElementById('estrix-result-status');
    const resultOutput = document.getElementById('estrix-result-output');
    if (resultSection) resultSection.classList.remove('estrix-hidden');

    if (result.success) {
      if (resultStatus) {
        resultStatus.textContent = '✅ JS 脚本执行成功';
        resultStatus.className = 'estrix-result-status success';
      }
      if (resultOutput) {
      ...
      if (resultStatus) {
        ...
        resultStatus.className = 'estrix-result-status error';
      }
      if (resultOutput) {
    ...
    console.error('[Estrix Code] JS 工具脚本执行异常:', err);
    const resultSection = document.getElementById('estrix-result-section');
    const resultStatus = document.getElementById('estrix-result-status');
    const resultOutput = document.getElementById('estrix-result-output');
    if (resultSection) resultSection.classList.remove('estrix-hidden');
...
async function handleToolCall(toolCall) {
  ...
  console.log(`[Estrix Code] 执行工具: ${toolName}`, params);
  ...
  try {
    ...
    console.error('[Estrix Code] 工具执行异常:', err);
    const resultSection = document.getElementById('estrix-result-section');
    const resultStatus = document.getElementById('estrix-result-status');
    const resultOutput = document.getElementById('estrix-result-output');
    if (resultSection) resultSection.classList.remove('estrix-hidden');
