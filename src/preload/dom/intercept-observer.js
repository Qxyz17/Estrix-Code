 ...
 * 监听主世界注入的 'estrix-ai-response' 事件，收到完整回复后走与 DOM 模式
 * 相同的工具调用/JS 代码块处理流程。
 */
...
async function processInterceptedResponse(text) {
  ...
  if (toolCall) {
    ...
    if (!hasTool(toolCall.toolName)) {
      console.log('[Estrix Code][拦截] 工具不存在: ' + toolCall.toolName);
  ...
  if (hasAntmlXml || (hasXmlInvoke && (hasXmlClose || hasXmlParam))) {
    if (xmlHintCount >= XML_HINT_MAX) {
      console.log('[Estrix Code][拦截] 已连续提示 ' + xmlHintCount + ' 次 XML 格式，停止发送');
      return;
    }
    xmlHintCount++;
    console.log('[Estrix Code][拦截] 检测到 XML 格式工具调用（第 ' + xmlHintCount + ' 次提示）');
    sendMessageToChat(
  ...
  // 4. 普通文本回复
  console.log('[Estrix Code][拦截] 正常文本回复，未检测到工具调用');
  try {
...
function startInterceptObserver() {
  window.addEventListener('estrix-ai-response', (ev) => {
    try {
      const detail = ev && ev.detail;
      if (!detail || !detail.finished) return;
      processInterceptedResponse(detail.text);
    } catch (err) {
      console.error('[Estrix Code][拦截] 处理回复事件出错:', err);
    }
  });
  console.log('[Estrix Code][拦截] 已启动 estrix-ai-response 事件监听');
}
