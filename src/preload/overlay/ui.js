...
/**
 * 注入覆盖层 HTML 到页面 body
 * 创建 estrix-root 容器并填充 OVERLAY_HTML 内容
 */
...
function showToast(text, duration = 2200) {
  let toast = document.getElementById('estrix-toast');
  if (!toast) {
    toast = document.createElement('div');
    toast.id = 'estrix-toast';
    toast.className = 'estrix-toast';
...
function showConfirmDialog(text, options) {
  ...
  // 移除旧弹窗
  const old = document.getElementById('estrix-confirm-dialog');
  if (old) old.remove();

  return new Promise((resolve) => {
    const dialog = document.createElement('div');
    dialog.id = 'estrix-confirm-dialog';
    ...
    dialog.querySelector('#estrix-confirm-ok').addEventListener('click', () => {
      cleanup();
      resolve(true);
    });
    if (showCancel) {
      dialog.querySelector('#estrix-confirm-cancel').addEventListener('click', () => {
        cleanup();
        resolve(false);
      });
    }
...
function setTaskStatus(running) {
  const status = document.getElementById('estrix-task-status');
  if (status) {
...
function showOverlay() {
  const el = document.getElementById('estrix-overlay');
  if (el) el.classList.remove('estrix-hidden');
}
...
function hideOverlay() {
  const el = document.getElementById('estrix-overlay');
  if (el) el.classList.add('estrix-hidden');
}
...
function displayCommand(cmdData) {
  ...
  if (resultSection) resultSection.classList.add('estrix-hidden');
  showToast('发现可执行的命令');
}
...
function renderHistory() {
  const list = document.getElementById('estrix-history-list');
  if (!list) return;
  ...
  list.innerHTML = items.map((item) => `
    <div class="estrix-history-item" data-id="${escapeHtml(item.id)}">
      <span class="estrix-cmd-text">${escapeHtml(truncate(item.command, 60))}</span>
      <span class="estrix-cmd-status ${item.canceled ? '' : item.success ? 'success' : 'error'}">
        ${item.canceled ? '⏹ 已忽略' : item.success ? '✅ 成功' : '❌ 失败'}
      </span>
      <span class="estrix-cmd-time">${formatTime(item.timestamp)}</span>
    </div>
  `).join('');

  list.querySelectorAll('.estrix-history-item').forEach((el) => {
    el.addEventListener('click', () => {
      ...
      if (entry) {
        ...
        if (entry.output && resultSection) {
          resultSection.classList.remove('estrix-hidden');
          if (resultStatus) {
            resultStatus.textContent = entry.canceled ? '⏹ 已忽略' : entry.success ? '✅ 执行成功' : '❌ 执行失败';
            resultStatus.className = `estrix-result-status ${entry.success ? 'success' : 'error'}`;
          }
...
function flashBadge() {
  ...
  if (dot) {
    ...
    setTimeout(() => {
      ...
      dot.style.animation = 'estrix-pulse 2s infinite';
    }, 3000);
  }
}
...
function updateHomeMode() {
  ...
  if (overlay) {
    if (isHome) {
      overlay.classList.add('estrix-home-mode');
      showFirstTimeDialog();
    } else {
      overlay.classList.remove('estrix-home-mode');
      hideFirstTimeDialog();
    }
  }
}
...
function showFirstTimeDialog() {
  ...
  const dialog = document.getElementById('estrix-first-time-dialog');
  if (dialog) dialog.classList.remove('estrix-hidden');
}
...
function hideFirstTimeDialog() {
  const dialog = document.getElementById('estrix-first-time-dialog');
  if (dialog) dialog.classList.add('estrix-hidden');
}
...
function forceShowOverlay() {
  const overlay = document.getElementById('estrix-overlay');
  if (overlay) {
    overlay.classList.remove('estrix-hidden');
