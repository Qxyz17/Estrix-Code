...
test('FileEditTool 替换文本', async () => {
  ...
  const r = await tool.execute({ file_path: 'a.txt', old_string: 'world', new_string: 'estrix', projectDir: tmpRoot });
  assert.strictEqual(r.success, true);
  assert.strictEqual(fs.readFileSync(path.join(tmpRoot, 'a.txt'), 'utf8'), 'hello estrix');
});
...
test('EditTool 成功替换', async () => {
  ...
  const r = await tool.execute({ file_path: 'd.txt', old_string: 'world', new_string: 'estrix', projectDir: tmpRoot });
  assert.strictEqual(r.success, true);
  assert.match(r.data, /updated successfully/);
  assert.strictEqual(fs.readFileSync(path.join(tmpRoot, 'd.txt'), 'utf8'), 'hello estrix');
});
...
test('EditTool dry-run 唯一替换预览', async () => {
  fs.writeFileSync(path.join(tmpRoot, 'd.txt'), 'hello world');
  const tool = new EditTool();
  const r = await tool.execute({ file_path: 'd.txt', old_string: 'world', new_string: 'estrix', dryRun: true, projectDir: tmpRoot });
  assert.strictEqual(r.success, true);
  assert.match(r.data, /DRY-RUN/);
