...
## [0.2.3-alpha] - 2026-09-13

### Changed
- 应用图标改为纯色终端 `>_`（无描边）：打包用白色图标；窗口/任务栏图标跟随系统主题（深色→白，浅色→黑）


## [0.2.2-alpha] - 2026-09-13

### Changed
- 应用图标加白色描边，深色/浅色主题下均清晰可见
- Release 说明改用 Markdown 模板（含下载指引 + 从 CHANGELOG 提取本版更新）

## [0.2.1-alpha.2] - 2026-09-13

### Changed
- **移除 Linux 构建**，仅保留 Windows / macOS（Linux 交由社区）

### Fixed
- 修复标签页模式下新建标签无反应：账号选择浮层被 40px 标签栏视图裁剪，改用原生菜单
- 修复 asarUnpack 未解包 macOS 的 ripgrep 二进制
- 开发环境下检查更新给出明确提示

## [0.2.0-alpha] - 2026-09-13

### Added
- **标签页式多账号管理**：单窗口 + 顶部标签栏（切换 / 关闭 / 新建 / 拖拽排序），内容使用 WebContentsView
- 每个 `(平台, 账号)` 使用独立 partition，登录态（cookies/session）自动持久化，重启保持
- 点 `+` 弹出账号选择菜单（按平台分组、标记上次账号），同一账号可开多个标签
- 关闭标签仅关闭界面，账号与登录态保留；删除账号则清除 profile 与 partition 数据
...
- 渲染进程可通过 --estrix-user-data 参数加载自定义 Provider
...
- 工具 API 类型定义从 `tools/estrix-tools.d.ts` 动态读取，避免多处维护
...
- 移除 .estrixCode/ESTRIX.md 作为全局项目介绍（避免干扰其他项目场景）
...
- 支持危险命令检测与额外警告
- 会话持久化（cookies/localStorage 保存到 %APPDATA%/estrix-code-session）
